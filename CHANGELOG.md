...
Every place that shows the owner's name — page title, welcome modal eyebrow,
send button, send modal header — now uses a fully translated phrase in the
visitor's language.

#### Translations added

| Language | Template | Example output |
|---|---|---|
| 🇬🇧 English | `Message to…` | Message to Paul Fleury |
| 🇫🇷 French | `Message pour …` | Message pour Paul Fleury |
| 🇩🇪 German | `Nachricht an …` | Nachricht an Paul Fleury |
| 🇮🇹 Italian | `Messaggio per …` | Messaggio per Paul Fleury |
| 🇪🇸 Spanish | `Mensaje para …` | Mensaje para Paul Fleury |
| 🇳🇱 Dutch | `Bericht aan …` | Bericht aan Paul Fleury |
| 🇨🇳 Chinese | `致…的消息` | 致Paul Fleury的消息 |
| 🇮🇳 Hindi | `… के लिए संदेश` | Paul Fleury के लिए संदेश |
| 🇯🇵 Japanese | `…へのメッセージ` | Paul Fle​uryへのメッセージ |
| 🇷🇺 Russian | `Сообщение для…` | Сообщение для Paul Fleury |

The `…` token is replaced at runtime with the owner's display name.
Languages where the name naturally precedes the phrase (Hindi, Japanese)
use `… के लिए संदेश` / `…へのメッセージ` — the `\s?…` regex in the
replace call correctly handles leading vs trailing position.

#### Also fixed

- `send_to_paul` templates updated for all 10 languages to use the same
  translated prefix pattern (`Message pour…`, `Nachricht an…`, etc.)
- `send_modal_title` likewise updated
- Name-change handler (settings display name update) now uses language
  templates everywhere — no more hardcoded English on live rename
- Roadmap "Live now" chip updated to v5.2.6
- All version headers → 5.2.6
...
See release notes on [GitHub](https://github.com/paulfxyz/hollr/releases/tag/v5.2.5).

---

## [5.2.4] — 2026-04-10

See release notes on [GitHub](https://github.com/paulfxyz/hollr/releases/tag/v5.2.4).

---

## [5.2.3] — 2026-04-10

See release notes on [GitHub](https://github.com/paulfxyz/hollr/releases/tag/v5.2.3).

---

## [5.2.2] — 2026-04-10

See release notes on [GitHub](https://github.com/paulfxyz/hollr/releases/tag/v5.2.2).

---

## [5.2.1] — 2026-04-10

### Full i18n audit — 10 languages corrected

This release fixes every untranslated, misattributed, or wrong-language string
across all 10 supported locales (EN, FR, DE, IT, ES, NL, ZH, HI, JA, RU).

#### Fixed — `handle/index.html`

**Strings in wrong language (copy-paste contamination)**
Previous patch sessions accidentally left the French translations of several keys
in all non-French language blocks, and Russian strings in all English blocks.

| Key | Was | Fixed to |
|---|---|---|
| `how_reach` (EN–NL, ZH, HI, JA) | `'Как с вами связаться'` (Russian) | Correct per-language translation |
| `page_sub` (DE, IT, ES, NL, ZH, HI, JA) | French text | Correct per-language translation |
| `step3` (DE, IT, ES, NL) | French `'Envoyez votre message…'` | Correct per-language translation |
| `welcome_desc2` (FR–NL, ZH, HI, JA, RU) | English original | Correct per-language translation |
| `send_modal_title` (FR–RU) | English `'Send message'` | Correct per-language translation |
| `modal_foot_note` (FR–RU) | English `'Message delivered securely…'` | Correct per-language translation |
| `go_to_paul` (ZH) | English `'Back to hollr.to'` | `'返回 hollr.to'` |

**Hardcoded owner name references (RU block)**
- `welcome_eyebrow`: `'Сообщение Полю Флёри'` → `'Сообщение для…'`
- `welcome_desc2`: referenced Paul by name → generic recipient
- `page_sub`: referenced Paul by name → generic copy
- `send_modal_title`: `'Отправить Полю'` → `'Отправить сообщение'`
- `modal_foot_note`: referenced Resend + Paul → generic hollr delivery note

**Unescaped apostrophes (JS syntax)**
- FR `page_sub` and `step3` had raw single quotes inside single-quoted JS strings,
  causing `SyntaxError: Unexpected identifier` in affected language blocks.
  Fixed with `\'` escaping.

#### Fixed — `backend/mailer.js`

- **Bullet points in attachment email** — file list used `<ul><li>` which renders
  as `• bullet` in some email clients. Replaced with `<div>` rows — no bullets,
  consistent rendering across Gmail, Apple Mail, Outlook.

#### Fixed — `handle/index.html` (PIN UX)

- **"Session expired" when PIN cache cleared** — after a page reload the in-memory
  `settingsPinCache` is null (correct — PIN is never persisted for security).
  Previously this showed a cryptic "Session expired." error inline in settings.
  Now re-opens the PIN gate cleanly so the user can re-enter their PIN.
